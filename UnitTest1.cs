...
namespace EmployeePayrollTestCases
{
    ...
    public class UnitTest1
    {
        ...
        [TestMethod]
        public void GivenEmployee_WhenAddedTolist_shouldMathchEmployeeEntries()
        {
        ...
        public void GivenEmployee_WhenAddedToListwith_thread_ShouldMatchToEntriesInList()
        {
            List<EmployeeDetails> employeeDetails = createList();
            
            EmployeePayrollOpreation opreation = new EmployeePayrollOpreation();
            DateTime beforeinsertion = DateTime.Now;
            opreation.addEmployeeToPayrollWithThread(employeeDetails);
            DateTime afterinsertion = DateTime.Now;
            Console.WriteLine("Time Taken in insertion without threading in case of list is : ", (afterinsertion - beforeinsertion));
        }
        ...
        public void GivenEmployee_WhenAddedToListShould_AddedSynchronizationly()
        {
            List<EmployeeDetails> employeeDetails = createList();

            EmployeePayrollOpreation opreation = new EmployeePayrollOpreation();
            DateTime beforeinsertion = DateTime.Now;
            opreation.addEmployeeToPayrollWiththreadUsingSynchronizarion(employeeDetails);
            DateTime afterinsertion = DateTime.Now;
            Console.WriteLine("time taken during process: {0} ", (afterinsertion - beforeinsertion));
        }
        private static  List<EmployeeDetails> createList()
        {
